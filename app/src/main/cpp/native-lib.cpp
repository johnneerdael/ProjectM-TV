...
constexpr const char* kPresetIndex = "presets.idx";  // generated by tools/gen-preset-index.sh
...
private:
    ...
    // Reads the prebuilt preset list: one file name per line. False if it is missing or empty.
    bool ReadIndexFile(std::vector<std::string>& names) const {
        ...
        for (size_t i = 0; data && i <= length; ++i) {
            ...
            if (EndsWithMilk(line.c_str())) names.push_back(std::move(line));
        }
        AAsset_close(asset);
        return !names.empty();
    }
...
bool LoadPreset(const std::string& name, bool smooth) {
    ...
    LOGI("LOAD preset='%s' ms=%.0f smooth=%d", name.c_str(), loadMs, smooth ? 1 : 0);
