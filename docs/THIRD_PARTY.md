...
|---|---|---|
| projectM 4.1.7, built from source with the app: git submodule `third_party/projectm` at tag `v4.1.7` (commit `e0b0a967`, with projectm-eval as its own submodule), plus the patches in `tools/projectm-patches/` (0001: upstream commit `0227b7a6`, plasma transition fix for NVIDIA SHIELD; 0002: render target, frame-keeping resize, shader program cache, shader state caching and an outgoing-preset frame divisor; 0003: hlslparser uses the prebuilt "C" locale; 0004: projectm-eval's `rand()` state per thread); linked statically into `libprojectmtv.so`. Up to 1.9.4 the app shipped a prebuilt master snapshot, commit `d89c09ef` (August 2025); 1.9.5 shipped prebuilt 4.1.7. | [projectM](https://github.com/projectM-visualizer/projectm) | LGPL 2.1 |
