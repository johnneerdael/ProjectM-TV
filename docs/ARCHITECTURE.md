...
| The background compiler's second projectM instance costs memory (textures it loads), and memory pressure makes Android take it from the music player | Low | The instance exists only for the ~0.1–1 s of each compile, at a tiny window size; no compile while less than 15 % of RAM is available, and a 60 s pause after each `onTrimMemory` signal; the program cache is capped at 8 MB including its keys |
