...
## Fixed
- **Auto no longer stays stuck at a low resolution after a low-memory warning.** In 1.9 the lower resolution Auto picked when Android reported low memory was remembered, so every later launch started there (on the SHIELD: 720p). The limit now applies to that session only; the next launch starts at the level chosen for the frame rate. If you are already stuck at 720p, Auto moves back up on its own after about 15 seconds of smooth playback.
